...
using Daric.EndPoints.Web.Extentions;

namespace Daric.EndPoints.Web.Controllers;
...
public class BaseController : Controller
{
    ...
    {
        CommandResult<TCommandResult> result = await CommandDispatcher.Send<TCommand, TCommandResult>(command);
    ...
    {
        CommandResult  result = await CommandDispatcher.Send(command);
        if (result.Status == ApplicationServiceStatus.Ok)
        {
    ...
    {
        QueryResult<TQueryResult> result = await QueryDispatcher.Execute<TQuery, TQueryResult>(query);
